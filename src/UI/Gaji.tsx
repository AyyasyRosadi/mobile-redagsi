...
export default function Gaji() {
  ...
  useEffect(() => {
    if (getBulan !== null) {
      dispatch(getGaji({ nupy: "19890720141156", bulan: getBulan }))
    }
  }, [getBulan])
  useEffect(() => {
                    ...
                    <View className='mt-3'>
                      ...
                      <View className='ml-[7%] mt-3 mb-1'>
                        {allGaji.pinjaman_ptks.map((d: any, i: number) => (
                          <FieldGaji list id={i} title={d.ket} value={currency(d.potongan)} />
                        ))}
                        {allGaji.titipan_potongans.map((s: any, i: number) => (
                          <FieldGaji list id={i} title={s.ket} value={currency(s.nominal)} />
                        ))}
                      </View>
                      <View className='bg-slate-900 w-full h-[1%] mt-2 mb-4'></View>
                      <View className='-ml-[3%]'>
